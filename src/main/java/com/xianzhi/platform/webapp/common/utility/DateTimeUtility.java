package com.xianzhi.platform.webapp.common.utility;
...
public class DateTimeUtility {
    ...
    // 将时间按配置的格式转换为String
    public static String formatYYYYMMDDHHMMSS_NOSPACE(Date date) {
        if (date == null) {
            return "";
        }
    ...
    public static String formatMMDD(Date date) {
        ...
        return getDateFormat(DateFormatType.MMDD).format(date);
    }
    ...
    public static boolean isTheSameDate(Date date1, Date date2) {
        ...
        String dateStr1 = formatYYYYMMDD(date1);
        String dateStr2 = formatYYYYMMDD(date2);
	 ...
	 * 	例如：endDate:2016-03-10 和startDate:2016-02-06
	 *			参数dayBased为true,则值为1；参数dayBased为false,则值为0；
	 * 		endDate:2017-03-10 和startDate:2016-03-06
	 *			参数dayBased为true,则值为2；参数dayBased为false,则值为1；
	 * @param start 开始日期
	 * @param end 结束日期
	 * @param dayBased true表示基于同时基于年份月份和天计算相差的年份，false表示只根据月年份计算
	 * @return
	 */
	public static int yearsBetween(Date start, Date end, boolean dayBased) {
        if (start.after(end)) {
            Date t = start;
            start = end;
            end = t;
	...
	public static void main(String[] args) throws Exception {
        ...
        Date begin = DateTimeUtility.parseYYYYMMDD("1995-03-07");
        Date end = DateTimeUtility.parseYYYYMMDD("1996-03-08");

        int i = yearsBetween(begin, end, false);
